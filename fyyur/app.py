...
def search_venues():
    ...
    #     response_data = dict(zip(('id', 'name'), artists))
    #     response['data'] = []
    #     for artist_data in Artist.query.filter_by(id=response_data['id'], name=response_data['name']).all():
    #         shows = Show.query.filter_by(artist_id=artist_data.id).all()
    #         upcoming_shows = [show for show in shows if show.start_time >= datetime.today()]
    #         response_data.update({'num_upcoming_shows': len(upcoming_shows)})
...
def artists():
    ...
    # data = db.session.query(Artist.id, Artist.name).distinct()
